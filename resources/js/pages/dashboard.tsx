                    ...
                    >
                        ...
                        <div className="divide-y divide-slate-100 dark:divide-slate-700">
                            {[...Array(4)].map((_, rowIndex) => (
                                <div key={rowIndex} className="flex items-center justify-between px-6 py-3">
...
export default function Dashboard({ products, sales }: DataProps) {
    ...
    useLayoutEffect(() => {
        ...
        gsap.fromTo(
            ...
            { scaleY: 0 },
            { scaleY: 1, duration: 0.5, ease: 'power3.out', stagger: 0.05, transformOrigin: 'bottom' },
        );
    }, [loading, chartView]);
    ...
    useEffect(() => {
        ...
        gsap.fromTo(
            ...
            { scaleX: 0 },
            { scaleX: 1, duration: 0.6, ease: 'power3.out', stagger: 0.08, transformOrigin: 'left', delay: 0.3 },
        );
    }, [loading]);
    ...
    const totalStockValue = products.reduce((sum, product) => sum + Number(product.wholesale_price) * Number(product.stock_quantity), 0);
    const totalOrders = sales.reduce((sum, sale) => sum + (sale.products?.reduce((s: number, p: any) => s + (p.pivot?.quantity || 0), 0) || 0), 0);
    ...
    const salesByMonth = [...Array(12)].map((_, i) => {
        ...
        return {
            ...
            revenue: monthSales.reduce((sum, sale) => sum + Number(sale.total), 0),
        };
    });
    ...
    // Active chart
    const activeChartData = chartView === 'week'
            ...
            <div className="mx-4 space-y-6 p-6">
                ...
                <div className="grid grid-cols-4 gap-4">
                    <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-600/10">
                        ...
                        <p className="mb-1 text-xs font-semibold tracking-wider text-slate-400 uppercase dark:text-slate-300/60">Total Revenue</p>
                        <p className="font-mono text-2xl font-bold text-slate-900 dark:text-white">₱{totalRevenue.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</p>
                        <p className="mt-1 text-xs text-slate-400 dark:text-slate-300/70">Today: ₱{todayRevenue.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</p>
                    </div>
                    <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-600/10">
                        <div className="mb-3 flex items-center justify-between">
                        ...
                        <p className="mb-1 text-xs font-semibold tracking-wider text-slate-400 uppercase dark:text-slate-300/60">Total Sales</p>
                        <p className="font-mono text-2xl font-bold text-slate-900 dark:text-white">{totalSales}</p>
                        <p className="mt-1 text-xs text-slate-400 dark:text-slate-300/70">Total Orders: {totalOrders} items</p>
                    </div>
                    <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-600/10">
                        <div className="mb-3 flex items-center justify-between">
                        ...
                        <p className="mb-1 text-xs font-semibold tracking-wider text-slate-400 uppercase dark:text-slate-300/70">Total Products</p>
                        <p className="font-mono text-2xl font-bold text-slate-900 dark:text-white">{totalProducts}</p>
                        <p className="mt-1 text-xs text-slate-400 dark:text-slate-300/70">Stock Value: ₱{totalStockValue.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</p>
                    </div>
                    <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-600/10">
                        <div className="mb-3 flex items-center justify-between">
                        ...
                        <p className="mb-1 text-xs font-semibold tracking-wider text-slate-400 uppercase dark:text-slate-300/60">Stock Alerts</p>
                        <div className="flex items-baseline gap-2">
                            <p className="font-mono text-2xl font-bold text-red-600">{outOfStockProducts.length}</p>
                            <span className="text-xs text-red-500">Out</span>
                            <p className="ml-2 font-mono text-2xl font-bold text-amber-600">{lowStockProducts.length}</p>
                            <span className="text-xs text-amber-500">Low</span>
                        </div>
                        <p className="mt-1 text-xs text-slate-400 dark:text-slate-300/70">{inStockProducts.length} in stock</p>
                    </div>
                </div>

                {/* ── Charts ── */}
                <div className="grid grid-cols-3 gap-6">
                    <div ref={barChartCardRef} className="col-span-2 rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-600/10">
                        <div className="mb-6 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <BarChart3 className="h-5 w-5 text-slate-500" />
                                <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-300">
                                    Sales This {chartView === 'week' ? 'Week' : 'Month'}
                                </h3>
                            </div>
                                ...
                                >
                                    <div className="flex w-full flex-col items-center gap-1">
                                        <span className="text-xs font-medium text-slate-600 dark:text-slate-400">
                                            ₱{entry.revenue.toLocaleString('en-PH', { maximumFractionDigits: 0 })}
                                        </span>
                                        <div
                                    ...
                                    <span className="text-xs text-slate-400 dark:text-slate-300">{entry.label}</span>
                                </div>
                            ))}
                        </div>
                            ...
                            <div className="flex items-center justify-between text-sm">
                                ...
                                <span className="font-semibold text-slate-900 dark:text-slate-300">
                                    ₱{chartView === 'week'
                                        ...
                                        : currentMonthData.revenue.toLocaleString('en-PH', { minimumFractionDigits: 2 })}
                                </span>
                            </div>
                        </div>
                    </div>

                    <div ref={categoryChartCardRef} className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-600/10">
                        <div className="mb-6 flex items-center gap-2">
                            <PieChart className="h-5 w-5 text-slate-500 dark:text-slate-300" />
                            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-300">Products by Category</h3>
                        </div>
                        <div className="space-y-3">
                                ...
                                return (
                                    <div key={category} className="space-y-1">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className="font-medium text-slate-900 dark:text-slate-400">{count} ({percentage}%)</span>
                                        </div>
                                        <div className="h-2 w-full rounded-full bg-slate-100 dark:bg-slate-700">
                                            <div
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-600/10">
                        <div className="border-b border-slate-200 px-6 py-4 dark:border-slate-800">
                            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-300">Recent Sales</h3>
                        </div>
                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {recentSales.length === 0 ? (
                                <div className="px-6 py-8 text-center text-sm text-slate-400">No sales yet</div>
                            ) : (
                                recentSales.map((sale) => (
                                    <a key={sale.id} href="/sales" className="flex items-center justify-between px-6 py-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-900/40">
                                        <div>
                                            <p className="text-sm font-medium text-slate-900 dark:text-slate-300">{sale.invoice_number}</p>
                                            <p className="text-xs text-slate-400 dark:text-slate-300/70">{formatDate(sale.created_at)} {shortFormatTime(sale.created_at)}</p>
                                        </div>
                                        <p className="font-mono text-sm font-semibold text-slate-900 dark:text-slate-300">₱{Number(sale.total).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</p>
                                    </a>
                                ))
                            )}
                        </div>
                    ...
                    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-600/10">
                        <div className="border-b border-slate-200 px-6 py-4 dark:border-slate-800">
                            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-300">Top Products by Stock</h3>
                        </div>
                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {topProducts.length === 0 ? (
                                <div className="px-6 py-8 text-center text-sm text-slate-400 dark:text-slate-300">No products yet</div>
                            ) : (
                                topProducts.map((product, index) => (
                                    <a key={product.id} href="/products" className="flex items-center gap-3 px-6 py-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-900/40">
                                        <span className="w-5 text-xs font-bold text-slate-400 dark:text-slate-300">#{index + 1}</span>
                                        <div className="min-w-0 flex-1">
                                            <p className="truncate text-sm font-medium text-slate-900 dark:text-slate-300">{product.product_name}</p>
                                            <p className="text-xs text-slate-400 dark:text-slate-400/80">{product.category?.category_name || 'No Category'}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="font-mono text-sm font-semibold text-slate-900 dark:text-slate-400">{product.stock_quantity}</p>
                                            <p className="text-xs text-slate-400 dark:text-slate-300/60">units</p>
                                        </div>
                                    </a>
                                ))
                            )}
